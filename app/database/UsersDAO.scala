...
class UsersDAO @Inject() (
  ...
  def getAuthRequestByToken(token: String): Future[Option[AuthRequest]] = {
    val query = AuthRequests.authRequests
      .filter(_.verificationToken === token)
      .result
      .headOption
    db.run(query)
  }
  ...
  def setAuthVerificationToken(id: UUID, token: String): Future[Any] = {
    val query =
      for (
        dbAuthRequest <- AuthRequests.authRequests if dbAuthRequest.id === id
      )
...
object AuthRequests {
  class AuthRequests(tag: Tag)
      extends Table[AuthRequest](tag, "auth_requests") {
    def id = column[UUID]("id", O.PrimaryKey)
    def address = column[String]("address")
    ...
    def * = (
      ...
      signingMessage,
      verificationToken
    ) <> ((AuthRequest.apply _).tupled, AuthRequest.unapply)
  }
